...
bool h264yuvWrite = false;
static struct WWC2_H264_STREAM_THREAD *quartH264Stream = NULL;
...
static void sigioHandler(int signum)
{
	...
	struct WWC2_H264_STREAM_THREAD *stream = NULL;
	...
	switch(mode)
	{
		...
		case WWC2_H264:
			...
			else if(action.act <= QUART_H264STREAM)
			{
				obj->h264Mode= (H264_MODE)action.act;
				switch(obj->h264Mode)
				{
					case FRONT_H264STREAM:
						stream = frontH264Stream;
						break;
					case BACK_H264STREAM:
						stream = backH264Stream;
						break;
					case LEFT_H264STREAM:
						stream = leftH264Stream;
						break;
					case RIGHT_H264STREAM:
						stream = rightH264Stream;
						break;
					case QUART_H264STREAM:
						stream = quartH264Stream;
						break;
					default:
						break;
				}

				if(stream != NULL && stream->yuvData == NULL)
				{
					sem_post(&stream->threadSem);
				}
			}
...
static void* h264StreamThreadFunc(void* arg)
{
	...
	while(1)
	{
		...
		if(thread->yuvData == NULL)
		{
			...
			if(pr2100Obj)
			{
				if(pr2100Obj->h264Mode == QUART_H264STREAM)
					pr2100Obj->h264Mode = QUART_H264;
				else
					pr2100Obj->h264Mode = FOUR_H264;
			}
		}
...
void pr2100_record_init(void)
{
	...
	if(recordEnable)
	{
		...
		quartH264Stream = h264_stream_thread_init(QUART_H264);
...
void pr2100_record_uninit(void)
{
	...
	if(obj != NULL)
	{
		...
		h264_stream_thread_uninit(quartH264Stream);
