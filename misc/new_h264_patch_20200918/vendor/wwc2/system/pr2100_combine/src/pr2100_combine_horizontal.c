...
void display_data_copy(unsigned char* dst, unsigned int width, unsigned int height)
{
	...
	{
		pthread_mutex_lock(&pr2100Obj->displayMutex);
		if(pr2100Obj->displaySize == DUAL_FHD_FRAME_YUV420_SIZE)
...
static void pr2100_record_action(struct FOUR_CHANNEL_FRAME_INFO *frameInfo, struct FOUR_CHANNEL_FRAME_INFO *pFrameInfo, unsigned char *dst, RECORD_MODE mode)
{
	if(mode == FOUR_RECORD)
	{
		...
		{
			pr2100_record_one_of_four_channel_hd(frameInfo, pFrameInfo, frontThread->data, FRONT_RECORD);
			sem_post(&frontThread->sem.recordReadSem);
		}
		if(sem_trywait(&backThread->sem.recordWriteSem) == 0)
		{
			pr2100_record_one_of_four_channel_hd(frameInfo, pFrameInfo, backThread->data, BACK_RECORD);
			sem_post(&backThread->sem.recordReadSem);
		}
		if(sem_trywait(&leftThread->sem.recordWriteSem) == 0)
		{
			pr2100_record_one_of_four_channel_hd(frameInfo, pFrameInfo, leftThread->data, LEFT_RECORD);
			sem_post(&leftThread->sem.recordReadSem);
		}
		if(sem_trywait(&rightThread->sem.recordWriteSem) == 0)
		{
			pr2100_record_one_of_four_channel_hd(frameInfo, pFrameInfo, rightThread->data, RIGHT_RECORD);
...
extern bool h264yuvWrite;
static void pr2100_h264stream_action(struct FOUR_CHANNEL_FRAME_INFO *frameInfo, struct FOUR_CHANNEL_FRAME_INFO *pFrameInfo, unsigned char *dst, H264_MODE mode)
{
	if(mode == FOUR_H264)
	{
		h264yuvWrite = true;
		if(sem_trywait(&frontH264Stream->yuvWriteSem) == 0)
		{
			pr2100_360p_scale_yuv420(frameInfo, pFrameInfo, frontH264Stream->yuvData, FRONT_H264);
			sem_post(&frontH264Stream->yuvReadSem);
		}
		if(sem_trywait(&backH264Stream->yuvWriteSem) == 0)
		{
			pr2100_360p_scale_yuv420(frameInfo, pFrameInfo, backH264Stream->yuvData, BACK_H264);
			sem_post(&backH264Stream->yuvReadSem);
		}
		if(sem_trywait(&leftH264Stream->yuvWriteSem) == 0)
		{
			pr2100_360p_scale_yuv420(frameInfo, pFrameInfo, leftH264Stream->yuvData, LEFT_H264);
			sem_post(&leftH264Stream->yuvReadSem);
		}
		if(sem_trywait(&rightH264Stream->yuvWriteSem) == 0)
		{
	...
	if(sem_trywait(&pr2100Obj->h264StreamThread.yuvWriteSem) == 0)
	{
		h264yuvWrite = true;
		switch(mode)
		{
			...
			case LEFT_H264:
			case RIGHT_H264:
				pr2100_360p_scale_yuv420(frameInfo, pFrameInfo,dst,mode);
				break;
			case QUART_H264:
				pr2100_360p_scale_quart_yuv420(frameInfo, pFrameInfo,dst,FRONT_H264);
				pr2100_360p_scale_quart_yuv420(frameInfo, pFrameInfo,dst,BACK_H264);
				pr2100_360p_scale_quart_yuv420(frameInfo, pFrameInfo,dst,LEFT_H264);
				pr2100_360p_scale_quart_yuv420(frameInfo, pFrameInfo,dst,RIGHT_H264);
				break;
			default:
				break;
		}
		sem_post(&pr2100Obj->h264StreamThread.yuvReadSem);
		h264yuvWrite = false;
	}
}
...
static void pr2100_combine_four_hd(unsigned char *src, const unsigned int width, const unsigned int height)
{
	...
	pr2100_h264stream_action(curFrameInfo, preFrameInfo, pr2100Obj->h264Stream, get_camera_h264_mode());
